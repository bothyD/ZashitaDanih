...
class Server:
    def __init__(self, host: str = "localhost", port: int = 3000):
        ...
        try:
            ...
            print("[INFO] Загружена база c пользователями")
        except FileNotFoundError:
            with open("registered_users.json", "w") as f:
                self._registered_users = dict()
                json.dump(self._registered_users, f)
            print(
                "[ERROR] He удалось загрузить базу c пользователями. Была создана новая")
        ...
        while True:
            ...
            if data == "{register}":
                ...
                if name not in self._registered_users.keys():
                    print(f"[INFO] Пользователь {addr} зарегистрировался c никнеймом '{name}' и ключом {v}")
