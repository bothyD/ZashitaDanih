...
class Client:
    ...
    def register(self, host: str = "localhost", port: int = 3000):
        print(f"[INFO] Попытка соединения c сервером для регистрации пользователя c ником '{self.name}'...")
        sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        sock.connect((host, port))
        print(f"[INFO] Соединение c сервером установлено")
        n = int(sock.recv(self._buffer_size).decode("utf8"))
        print(f"[INFO] c сервера получено n")
        ...
        if status == "success":
            ...
            print(f"Пользователь c никнеймом '{self.name}' уже зарегистрирован")
        
        print(f"[INFO] Соединение c сервером разорвано\n")
        sock.close()

    def auth(self, host: str = "localhost", port: int = 3000):
        print(f"[INFO] Попытка соединения c сервером для авторизации пользователя c ником '{self.name}'...")
        sock = socket.socket()
        sock.connect((host, port))
        print(f"[INFO] Соединение c сервером установлено")
        ...
        while True:
            ...
            if status == "success":
                ...
                print("B авторизации отказано. Аутентификация не пройдена")
                break
            elif status == "check":
                pass
